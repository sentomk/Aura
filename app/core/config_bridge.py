...
def bridge_config_to_env():
    ...
    try:
        ...
        # 桥接 MongoDB/Redis 开关和连接信息，供 tradingagents 内部缓存/统计模块使用。
        # .env 中通常只有拆分字段（MONGODB_HOST/PORT/USERNAME...），而
        # tradingagents.config.mongodb_storage 只认 MONGODB_CONNECTION_STRING。
        os.environ["MONGODB_ENABLED"] = os.getenv("MONGODB_ENABLED", "true")
        os.environ["REDIS_ENABLED"] = os.getenv("REDIS_ENABLED", "true")
        ...
        if not mongodb_conn_str:
            ...
            logger.info("  ✓ 从 Settings.MONGO_URI 生成 MONGODB_CONNECTION_STRING")
        else:
            os.environ["MONGODB_CONNECTION_STRING"] = mongodb_conn_str
        ...
        try:
            ...
            from tradingagents.config import database_manager as ta_database_manager
            ...
            if use_mongodb.lower() == "true":
                try:
                    # database_manager 可能在桥接前已经以 MONGODB_ENABLED=false 初始化过。
                    # 清掉全局单例，让后续缓存适配器用最新环境变量重建连接。
                    ta_database_manager._database_manager = None
