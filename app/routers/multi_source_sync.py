...
@router.post("/quotes/run")
async def run_quotes_sync():
    """手动运行行情数据同步（使用AKShare批量获取全市场实时行情）"""
    try:
        ...
        if not provider.is_available():
            raise HTTPException(status_code=503, detail="AKShare数据源不可用")
        ...
        if not codes:
            return SyncResponse(
                success=False, message="没有股票基础数据，请先同步基础数据", data={}
            )

        logger.info("同步行情: %s 只股票", len(codes))

        batch_size = 100
        total_ok = 0
        tz = ZoneInfo(getattr(settings, "TIMEZONE", "Asia/Shanghai"))
        now = datetime.now(tz)
            ...
            try:
                ...
                for code, q in quotes.items():
                    if not q:
                        continue
                    doc = {
                        "code": code.zfill(6), "symbol": code.zfill(6),
                        "name": q.get("name", ""),
                        ...
                        "amount": q.get("amount"), "pe": q.get("pe"),
                        "pb": q.get("pb"), "pe_ttm": q.get("pe_ttm"),
                        "updated_at": now,
                    }
                ...
                logger.error("批量 %s 错误: %s", i // batch_size, e)
            if (i // batch_size + 1) % 10 == 0:
                logger.info(
                    "进度: %s/%s, 已写入: %s",
                    min(i + batch_size, len(codes)), len(codes), total_ok
                )

        count = await db.market_quotes.count_documents({})
        logger.info("行情同步完成: %s 条, market_quotes 共 %s 条", total_ok, count)

        return SyncResponse(
            success=True,
            message=f"行情同步完成: 成功写入 {total_ok} 条",
            data={
                "total_stocks": len(codes),
                "synced": total_ok,
                "market_quotes_count": count,
            },
        )
        ...
        logger.error("行情同步失败: %s", e)
        raise HTTPException(status_code=500, detail=f"行情同步失败: {str(e)}")
