...
class AKShareSyncService:
    ...
    async def sync_realtime_quotes(self, symbols: List[str] = None, force: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            # 单只股票也使用批量接口（_get_and_save_quotes 的单个接口可能返回空）
            if len(symbols) == 1:
                logger.info("单个股票同步，使用批量接口获取")
                symbol = symbols[0]
                quotes_map = await self.provider.get_batch_stock_quotes([symbol])
                if quotes_map and symbol in quotes_map:
                    q = quotes_map[symbol]
                    if q:
                        quotes_data = {
                        ...
                        quotes_data = {k: v for k, v in quotes_data.items() if v is not None}
