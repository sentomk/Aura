...
from app.services.iwencai_news_provider import get_iwencai_provider
...
class NewsDataSyncService:
    ...
    async def sync_stock_news(
        ...
        try:
            ...
            if data_sources is None:
                data_sources = ["tushare", "akshare", "realtime", "iwencai"]
            ...
            # 4. 同花顺问财新闻
            if "iwencai" in data_sources:
                try:
                    iwencai_news = await self._sync_iwencai_news(
                        symbol, hours_back, max_news_per_source
                    )
                    if iwencai_news:
                        all_news.extend(iwencai_news)
                        stats.sources_used.append("iwencai")
                        self.logger.info(f"✅ 问财新闻获取成功: {len(iwencai_news)}条")
                except Exception as e:
                    self.logger.error(f"❌ 问财新闻获取失败: {e}")
    ...
    async def _sync_iwencai_news(
        ...
        """同步同花顺问财新闻"""
        try:
            provider = await get_iwencai_provider()
            ...
            if news_data:
                standardized_news = []
                for news in news_data:
                    standardized = self._standardize_iwencai_news(news, symbol)
                    if standardized:
                        standardized_news.append(standardized)
                return standardized_news
            ...
            self.logger.error(f"❌ 问财新闻同步失败: {e}")
            return []

    def _standardize_iwencai_news(
        ...
        try:
            ...
            return {
                ...
                "url": news.get("url", ""),
                "source": news.get("source", "同花顺问财"),
                "author": "",
                "publish_time": news.get("publish_time", ""),
                "category": self._classify_news_category(title),
            ...
            self.logger.error(f"❌ 标准化问财新闻失败: {e}")
            return None
