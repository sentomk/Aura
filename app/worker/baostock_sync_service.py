...
class BaoStockSyncService:
    ...
    async def sync_single_historical_data(
        ...
        """同步单只股票历史数据，用于分析前的数据补齐。"""
        try:
            if self.db is None or self.historical_service is None:
            ...
            if hist_data is None or hist_data.empty:
                return {
                    "success_count": 0,
                    "total_records": 0,
                    "errors": [{"code": symbol, "error": "历史数据为空"}],
                }
            ...
            return {
                ...
                "errors": [] if saved_count > 0 else [{"code": symbol, "error": "保存历史数据失败"}],
            }
        except Exception as e:
            logger.error(f"❌ BaoStock单股历史数据同步失败: {symbol}, {e}", exc_info=True)
            return {
                "success_count": 0,
                "total_records": 0,
                "errors": [{"code": symbol, "error": str(e)}],
            }
