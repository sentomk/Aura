        ...
        if doc and "llm_configs" in doc:
            ...
            for config_dict in llm_configs:
                if config_dict.get("model_name") == model_name:
                    ...
                    api_key = None
                    if model_api_key and model_api_key.strip() and not _is_placeholder_api_key(model_api_key):
                        ...
                        if provider_api_key and provider_api_key.strip() and not _is_placeholder_api_key(provider_api_key):
...
def _is_placeholder_api_key(api_key: str) -> bool:
    ...
    if not api_key or not api_key.strip():
        return True
    key = api_key.strip()
    if key.startswith("your-") or key.endswith("-key") and "your" in key.lower():
        return True
...
class SimpleAnalysisService:
    ...
    async def execute_analysis_background(
        ...
        # 🔧 使用 get_symbol() 方法获取股票代码（兼容 symbol 和 stock_code 字段）
        stock_code = request.get_symbol()
        if stock_code and not request.stock_code:
    ...
    async def list_user_tasks(
        ...
        try:
                ...
                async for doc in cursor:
                    ...
                    for k in ("start_time", "end_time"):
                        if item.get(k) and hasattr(item[k], "isoformat"):
                            dt = item[k]
                            # MongoDB stores naive datetimes as UTC. Mark them as UTC before returning to the frontend.
            ...
            # 🔥 统一处理时区信息（MongoDB naive datetimes are UTC）
            from datetime import timezone
                ...
                for time_field in ("start_time", "end_time", "created_at", "started_at", "completed_at"):
                    ...
                    if value:
                        ...
                        if hasattr(value, "isoformat"):
                            # 如果是 naive datetime，按 MongoDB UTC 时间标记
                        ...
                        # 如果是字符串且没有时区标识，添加时区标识
                            ...
                            if 'T' in value or ' ' in value:
                                task[time_field] = value.replace(' ', 'T') + '+00:00'
                ...
                "$or": [
                    {"started_at": {"$lt": cutoff_time}},
                    {
                        "$and": [
                            ...
                            {"$or": [{"started_at": None}, {"started_at": {"$exists": False}}]},
                        ]
                    },
                ]
            }
