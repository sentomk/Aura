...
class QuotesIngestionService:
    ...
    async def _backfill_from_akshare_realtime(self) -> None:
        """当历史数据为空时，直接用 AKShare 批量接口填充 market_quotes"""
        try:
            from tradingagents.dataflows.providers.china.akshare import get_akshare_provider
            provider = get_akshare_provider()
            if not provider.is_available():
                logger.warning("AKShare 不可用，无法回填行情")
                return
            db = get_mongo_db()
            cursor = db.stock_basic_info.find({}, {"code": 1})
            codes = [doc["code"] async for doc in cursor]
            if not codes:
                logger.warning("没有基础数据，无法回填行情")
                return
            logger.info("AKShare 实时回填: %s 只股票", len(codes))
            batch_size = 100
            total_ok = 0
            for j in range(0, len(codes), batch_size):
                ...
                try:
                    ...
                    logger.error("AKShare 回填批次错误: %s", e)
                if (j // batch_size + 1) % 10 == 0:
                    logger.info(
                        "AKShare 回填进度: %s/%s, 已写入: %s",
                        min(j + batch_size, len(codes)), len(codes), total_ok
                    )
            logger.info("AKShare 回填完成: %s 条", total_ok)
        except Exception as e:
            logger.error("AKShare 回填失败: %s", e)
    ...
    async def backfill_from_historical_data(self) -> None:
        ...
        try:
            ...
            if not docs:
                ...
                logger.warning("market_quotes 和历史数据集合都为空，回退到 AKShare 实时接口")
                await self._backfill_from_akshare_realtime()
