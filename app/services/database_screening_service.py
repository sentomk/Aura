...
class DatabaseScreeningService:
    ...
    async def screen_stocks(
        ...
        try:
            ...
            if total_count == 0:
                fallback = await self._screen_with_latest_quotes(
    ...
    async def _screen_with_latest_quotes(
        self,
        conditions: List[Dict[str, Any]],
        limit: int,
        offset: int,
        ...
        """Fallback to latest local quote rows when the screening view has no data."""
        quote_fields = {"pct_chg", "amount", "close", "volume", "code"}
        ...
        if any(field not in quote_fields for field in requested_fields):
            logger.info(f"ℹ️ [database_screening] 跳过行情兜底，不支持字段: {requested_fields - quote_fields}")
            return None

        db = get_mongo_db()
        collection = db["stock_daily_quotes"]
        latest_doc = await collection.find_one({}, sort=[("trade_date", -1)])
        if not latest_doc:
            return None
        ...
        if source:
            source_count = await collection.count_documents({**query, "data_source": source})
            if source_count > 0:
                query["data_source"] = source

        quote_query = await self._build_query(conditions)
        query.update(quote_query)
        logger.info(f"📋 [database_screening] 使用 stock_daily_quotes 兜底查询: {query}")
        ...
        async for doc in cursor:
            normalized = dict(doc)
            normalized["source"] = normalized.get("data_source") or normalized.get("source") or "stock_daily_quotes"
            results.append(self._format_result(normalized))

        logger.info(
            f"✅ [database_screening] 行情兜底筛选完成: 总数={total_count}, 返回={len(results)}, trade_date={latest_date}"
        )
        return results, total_count
