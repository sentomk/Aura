...
class DataSourceManager:
    ...
    def _get_data_source_priority_order(self, symbol: Optional[str] = None) -> List[ChinaDataSource]:
        ...
        try:
            ...
            if config_data and config_data.get('data_source_configs'):
                ...
                configured_source_types = {str(ds.get('type', '')).lower() for ds in data_source_configs if ds.get('type')}
                ...
                if (
                    'baostock' not in configured_source_types
                    and ChinaDataSource.BAOSTOCK in self.available_sources
                    and ChinaDataSource.BAOSTOCK not in result
                ):
    ...
    def _check_available_sources(self) -> List[ChinaDataSource]:
        ...
        try:
            ...
            if config_data and config_data.get('data_source_configs'):
                ...
                for ds in data_source_configs:
                    ds_type = ds.get('type', '').lower()
                    if ds_type:
                        configured_sources_in_db.add(ds_type)
        ...
        # 检查BaoStock
        if 'baostock' in enabled_sources_in_db or 'baostock' not in configured_sources_in_db:
            try:
